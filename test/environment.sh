...
@test "environment variables are set properly" {
 ...
 standard_hc_target_prefix=$( nix-shell --run 'echo $HC_TARGET_PREFIX' )
 [[ $standard_hc_target_prefix == $PWD ]]
 ...
 override_hc_target_prefix=$( NIX_ENV_PREFIX=foo nix-shell --run 'echo $HC_TARGET_PREFIX' )
 [[ $override_hc_target_prefix == 'foo' ]]
